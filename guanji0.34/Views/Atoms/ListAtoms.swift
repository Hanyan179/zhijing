...
public struct ListRow: View {
    ...
    public var body: some View {
        Button(action: { onClick?() }) {
            HStack {
                HStack(spacing: 12) {
                    Image(systemName: iconName).foregroundColor(Color(.systemGray))
                ...
                HStack(spacing: 6) {
                    ...
                    if hasArrow { Image(systemName: "chevron.right").foregroundColor(Color(.systemGray3)).font(.system(size: 12)) }
                }
            }
    ...
    public var body: some View {
        Button(action: { checked.toggle() }) {
            ZStack(alignment: checked ? .trailing : .leading) {
                Capsule().fill(checked ? Colors.slateDark : Color(.systemGray4)).frame(width: 40, height: 24)
