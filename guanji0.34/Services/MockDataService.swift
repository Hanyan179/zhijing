...
public enum MockDataService {
    ...
    public static func buildLocation(rawName: String, lat: Double, lng: Double) -> LocationVO {
        if let hitFence = fences.first(where: { abs($0.lat - lat) < 0.0001 && abs($0.lng - lng) < 0.0001 }) {
            if let mapping = mappings.first(where: { $0.id == hitFence.mappingId }) {
    ...
    public static let timeline: [String: [TimelineItem]] = {
        ...
        let journeyToday = JourneyBlock(type: "journey",
                                        ...
                                        duration: "42 min",
                                        entries: [
                                            JournalEntry(id: "e_today_music", type: .text, subType: nil, chronology: .present, content: "延安高架有点堵。这种灰色的天气，最适合听 Ryuichi Sakamoto 的《Opus》。感觉整个城市都在呼吸。", url: nil, timestamp: "09:10", category: .idea, metadata: nil)
                                        ])
