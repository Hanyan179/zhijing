...
public struct InputQuickActions: View {
    ...
    public var body: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 24) {
                ...
                // Mode toggle replaces "More" button - Requirements 6.1, 6.2
                if showModeToggle {
                    modeToggleButton()
                } else if showMore {
    ...
    /// Mode toggle button - shows brain icon in journal mode, book icon in AI mode
    /// Requirements: 6.1, 6.2, 6.3, 6.4
    private func modeToggleButton() -> some View {
        Button(action: {
            // Haptic feedback - Requirement 6.4
            #if canImport(UIKit)
            ...
            VStack(spacing: 4) {
                Image(systemName: currentMode == .journal ? "brain" : "book")
    ...
    private func quickActionButton(image: String, label: String, action: @escaping () -> Void, isDisabled: Bool = false) -> some View {
